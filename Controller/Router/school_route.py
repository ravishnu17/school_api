...
def school_profile_data(db:Session=Depends(db.get_db)):
...
def schoolData(id , db:Session = Depends(db.get_db)):
...
def updateSchoolProfile(id ,data:dict, db:Session = Depends(db.get_db)):
...
def insertNewProfile( data:dict , db:Session=Depends(db.get_db)):
    print(data)
